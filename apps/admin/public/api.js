...
(function(global){
  ...
  class LocalProvider {
    ...
    async listByLane(lane) {
      return this.dm.listByLane(lane);
    }

    async moveTask(taskId, lane, newIndex) {
      this.dm.moveWithinLane(taskId, lane, newIndex);
    }

    async promoteToThisWeek(taskId) {
      this.dm.promoteToThisWeek(taskId);
    }

    async promoteToToday(taskId) {
      this.dm.promoteToToday(taskId);
    }

    async setStatus(taskId, status) {
      this.dm.setStatus(taskId, status);
    }
  }
  ...
  class SupabaseProvider {
    ...
    async listByLane(lane) {
      // For Supabase, filter by lane field
      const res = await fetch(`${this.url}/rest/v1/tasks?select=*&lane=eq.${lane}&order=order_index.asc,created_at.asc`, {
        headers: this._headers()
      });
      if(!res.ok) throw new Error(`listByLane failed: ${res.status}`);
      return res.json();
    }
    ...
    async moveTask(taskId, lane, newIndex) {
      ...
      this.dm.moveWithinLane(taskId, lane, newIndex);
      // Note: Supabase update would go here in real implementation
    }
